<?php /* Portfolio Sprite Controller */ ?>

<?php  defined('C5_EXECUTE') or die("Access Denied.");

class PortfoliospriteBlockController extends BlockController {
	
	protected $btName = 'Portfolio Sprite';
	protected $btDescription = 'Add A Flying Image or text to the portfolio';
	protected $btTable = 'btportfoliosprite';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('PortfoliospriteBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('PortfoliospriteBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('PortfoliospriteBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('PortfoliospriteBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('PortfoliospriteBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('PortfoliospriteBlockController', 'replaceDownloadFileID'),
			$text);
